...
function newList(data) {
    ...
    if(data.列印狀態 == "列印完成"){
        ...
        <table class="table">
                    ...
                    <td style="width: 10%; text-align: center;">${data.部位}</td>
                    ...
                    <a href="CuttingDispatchListPDF.html?nfcid=${data.派工單編號}" target="_blank" style="text-decoration:none;">
    ...
    <table class="table">
                ...
                <a style="cursor: allowed; text-decoration:none;">
                    <button type="button" class="btn btn-outline-danger" name="${data.部位}" id="${time1}" value="${data.派工單編號}" onclick="delete_button(this.name, this.id, this.value)" abled>刪除派工單</button>
                </a>
                <!-- Modal -->
                <a href="QualityControl.html?orderNumber=${rid}&Cutting=${data.派工單編號}" style="text-decoration:none;">
                    <button type="button" class="btn btn-outline-primary" hidden>品質管理</button>
...
function printpdf(){
    var pdfip = 'http://'+ ip +'/CuttingMultiplepdf.html?nfcid';
    ...
    if (check.length == 1) pdfip = "http://127.0.0.1/CuttingDispatchListPDF.html?nfcid="+ check[0].value
    parent.open(pdfip);
