...
//分組派工點選時取得部位資訊
function getbuttonGroupdispatch(section) { //顯示內容
...
async function MaterialSection(number, section){
    ...
    await $.get('http://' + ip + '/api/GetHighFrequencyTableSectionInfo', {'rid': number, 'section': section}, function (data,status) {
    ...
    await $.get('http://' + ip + '/api/getMaterial1', {'orderNumber': rid, 'section': section}, function (datas,status) {
            console.log(datas)
...
function Newdata(section){
    ...
    $.get('http://' + ip + '/api/GetHighFrequencyTableSectionInfo', {'rid': rid, 'section': section}, function (data,status) {
        for (var i = 0; i < data.length; i++) {
...
function bns() {
    ...
    if (confirm(msg + " " + selectedEls.length) == true) {
        ...
        for (var i = 0; i < selectedEls.length; i++) {
            ...
            $.post('http://' + ip + '/api/UpdateHighFrequencyTable', { '數量': 0,'完成數量': 0,'派工時間': time2,'派工單編號': str,'製令單號': rid,'尺寸': size,'總數量': amount, '部位': selectSection}, function (res) {
            });
        ...
        alert("派工成功！");
        $.get('http://' + ip + '/api/GetHighFrequencyTableSectionInfo', {'rid': rid, 'section': selectSection}, function (data,status) {
                var percent = 0;
                var total = 0;
                console.log(data)
                for (var i = 0; i < data.length; i++) {
