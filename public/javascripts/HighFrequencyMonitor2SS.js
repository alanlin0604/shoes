...
async function Select(section){
  ...
  var size = [];
  var qty = [];
  await $.get('http://' + ip + '/api/GetHighFrequencyTableSectionInfo', {'rid': rid, 'section': section}, function (data,status) {
    for (var i = 0;i < data.length; i++){
      size.push(data[i].尺寸)
      if (data[i].完成數量 == null){
        qty.push(0)
...
async function Section(){
  ...
  $.get('http://' + ip + '/api/UpdateHighFrequencyCompletePercentx', {'rid': rid}, function (data,status) {
  })
  ...
  for(var i = 0; i < sectionlist.length; i++){
    ...
    $.get('http://' + ip + '/api/GetHighFrequencyPercent', {'rid': rid, 'section': sectionlist[i]}, function (data,status) {
  ...
  setTimeout(() => {
    $.get('http://' + ip + '/api/GetHighFrequencyTableSectionInfo', {'rid': rid, 'section': sectionlist[0]}, function (data,status) {
        for (var i = 0;i < data.length; i++){
  ...
  if (percent == null) {
    ...
    <tr>
      ...
      <td style="width: 30%; text-align: center;">
        <button type="button" class="btn btn-dark" disabled>詳細資訊</button>
      </td>
    </tr>
        `;
    ...
    $.get('http://' + ip + '/api/GetHighFrequencyItem', {'rid': rid}, function (data,status) {
        for(var i = 0; i < data.length; i++){
            ...
            <div class="row" style="margin-left: 50px;margin-top:8px;font-size: 19px;text-align: center;">
                ...
                <div class="col-2"><strong>工作內容：高周波</strong></div>
...
function PercentReload(){
  ...
  setTimeout(() => {
    ...
    console.log(sum)
    $.post('http://' + ip + '/api/UpdateHighFrequencyCompletePercent', { '完成狀態': sum, '製令單號': rid}, function (res) {
