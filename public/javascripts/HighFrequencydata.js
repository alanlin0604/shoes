...
function ShowData(number, amount){
    ...
    // }
        ...
        setTimeout(() => {
            $.get('http://' + ip + '/api/GetHighFrequencyTableInfo', {'workorderNumber': number}, function (data,status) {
                var rid = data[0].製令單號;
        ...
        $.get('http://' + ip + '/api/GetHighFrequencyTableList', {'製令單號': rid, '派工單編號': number}, function (data,status) {
...
async function delete_button(numbers) {
    ...
    var percent = 0
    if (confirm(msg) == true) {
        $.get('http://' + ip + '/api/GetHighFrequencyTableInfo', {'workorderNumber': numbers}, function (data,status) {
            for(var i = 0;i< data.length;i++){
                console.log(data[i]);
                $.get('http://' + ip + '/api/UndoHighFrequencyTable', { '數量': data[i].總數量, '完成數量': 0,'派工時間': null,'派工單編號': null, '製令單號': data[i].製令單號, '派工單編號old': numbers, '尺寸': data[i].尺寸}, function (res) {
