...
function newList(data){
    ...
    <div class="tab-content" id="${data.製令單號}">
        ...
        <table class="table">
                    ...
                    <td style="width: 10%;  text-align: center;">印刷</td>
                    ...
                    <td style="width: 25%; padding-left: 30px" >
                        <a href="PrintingDispatchSystem.html?orderNumber=${data.製令單號}" style="text-decoration:none;">
                            <button type="button" class="btn btn-outline-dark" >進入派工畫面</button>
                        </a>

                        <a href="PrintingDispatchList.html?workorderNumber=${data.製令單號}" style="text-decoration:none;">
...
function getList(){ //顯示內容
    $.get('http://' + ip + '/api/GetPrinting',function(data,status){
...
//         $.post('http://' + ip + '/api/UpdatePrintingSection', { '部位': sections, '製令單號': rid }, function (res) {
...
function DataCheck(){
    $.get('http://' + ip + '/api/gettable1', function (data,status) {
        for(var i = 0; i < data.length; i++){
            $.get('http://' + ip + '/api/GetPrintingTable', {'rid': data[i].製令單號}, function (data,status) {
                var percent = 0;
                var total = 0;
                console.log(data)
                for (var i = 0; i < data.length; i++) {
                ...
                setTimeout(() => {
                    $.post('http://' + ip + '/api/UpdatePrintingPercent', { '派工進度': percent, '製令單號': data[0].製令單號 }, function (res) {
