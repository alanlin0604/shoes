...
function GetTable(){
    ...
    setTimeout(() => {
        ...
        DispatchList.forEach(function(value) {
            $.get('http://' + ip + '/api/GetPrintingTableList', {'製令單號': rid, '派工單編號': value}, function (data,status) {
                newList(data[0]);
            })
          });
    },100)
...
function newList(data){
    ...
    if(data.列印狀態 == "列印完成"){
        ...
        <table class="table">
                    ...
                    <a href="PrintingDispatchListPDF.html?nfcid=${data.派工單編號}" target="_blank" style="text-decoration:none;">
                    ...
                    <!-- Modal -->
                    <!--<a href="QualityControl.html?orderNumber=${rid}&Printing=${data.派工單編號}" style="text-decoration:none;">
...
function printpdf(){
    var pdfip = 'http://'+ ip +'/PrintingMultiplePDF.html?nfcid';
    ...
    if (check.length == 1) pdfip = "http://127.0.0.1/PrintingDispatchListPDF.html?nfcid="+ check[0].value
    parent.open(pdfip);
...
function delete_WorkOrderNumber(WorkOrderNumber){
    $.get('http://' + ip + '/api/GetPrintingTableList', {'製令單號': rid, '派工單編號': WorkOrderNumber}, function (data,status) {
        for(var i = 0; i < data.length;i++){
            $.post('http://' + ip + '/api/UpdatePrintingTable', { '數量': data[i].總數量,'完成數量': 0,'派工時間': null,'派工單編號': null,'製令單號': rid,'尺寸': data[i].尺寸,'總數量': data[i].總數量}, function (res) {
