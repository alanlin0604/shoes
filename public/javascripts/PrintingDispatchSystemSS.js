...
function gettittleList(data) {
    ...
    getbuttonGroupdispatch(sectionlist[0]);
...
function gettittle() {
    ...
    // $.get('http://' + ip + '/api/GetPrintingTableSize', {'rid': rid}, function (data,status) {
    //     for(var i = 0; i < data.length; i++){
    //         sizelist.push(data[i].尺寸)
    //         amountlist.push(data[i].數量)
    //     }
    // })

    setTimeout(() => {
        $.get('http://' + ip + '/api/GetPrintingItem', {'rid': rid}, function (data,status) {
                gettittleList(data[0]);
            })
    }, 100)
...
//派工點選時取得部位資訊
async function getbuttonGroupdispatch(section) { //顯示內容
    ...
    for(var x = 0; x < sectionlist.length; x++){
        MaterialSection(rid, sectionlist[x])
    }
...
async function MaterialSection(number, section){
    var percent = 0;
    var total = 0;

    await $.get('http://' + ip + '/api/GetPrintingTableSectionInfo', {'rid': number, 'section': section}, function (data,status) {
        
        console.log(data)
        for (var j = 0; j < data.length; j++) {
            console.log(j)
            percent += parseInt(data[j].數量);
            total += parseInt(data[j].總數量);
        }
    ...
    await $.get('http://' + ip + '/api/getMaterial1', {'orderNumber': number, 'section': section}, function (datas,status) {
            console.log(datas);
            Choose(datas[0], percent);
        })
}
async function Choose(data, percent) {
    if (percent == 100) {
        var content =
            `
        <tr>
                        ...
                        <div class="row">
                            <div class="col-sm-11" style="padding-left:0px;">
                                <div class="progress">
                                    <div id="myprogressbar${data.部位名稱}" class="progress-bar bg-success" role="progressbar" style="width:${percent}% ; " aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                                </div>
                            </div>
                                <div id="progressbartext${data.部位名稱}" class="col-sm-1" style="font-size:10px;padding-left:0px;">${percent}%</div>
                        </div>
                    </div>    
                    <td style="width: 20%;">
                        </td>
        ...
        <tr>
                        ...
                        <td style="width: 10%;">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="test1" id="flexRadioDefault1" value="${data.部位名稱}" onchange="Newdata(this.value)">
                                <label class="form-check-label" for="flexRadioDefault1">
                                    ${data.部位名稱}
                                </label>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-sm-11" style="padding-left:0px;">
                            <div class="progress">
                                <div id="myprogressbar${data.部位名稱}" class="progress-bar" role="progressbar" style="width:${percent}% ; " aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                        </div>
                            <div id="progressbartext${data.部位名稱}" class="col-sm-1" style="font-size:10px;padding-left:0px;">${percent}%</div>
                    </div>
...
function bns() {
    ...
    if (confirm(msg) == true) {
        ...
        for (var i = 0; i < selectedEls.length; i++) {
            ...
            $.post('http://' + ip + '/api/UpdatePrintingTable', { '數量': 0,'完成數量': 0,'派工時間': time2,'派工單編號': str,'製令單號': rid,'尺寸': size,'總數量': amount, '部位': selectSection}, function (res) {
            });
        ...
        $.get('http://' + ip + '/api/GetPrintingTableSectionInfo', {'rid': rid, 'section': selectSection}, function (data,status) {
                var percent = 0;
                var total = 0;
                console.log(data)
                for (var i = 0; i < data.length; i++) {
                ...
                document.getElementById("myprogressbar" + selectSection).style.width = percent + "%";
                document.getElementById("progressbartext" + selectSection).innerText= percent+"%"

                setTimeout(() => {
                    $.post('http://' + ip + '/api/UpdatePrintingPercent', { '派工進度': percent, '製令單號': rid }, function (res) {
