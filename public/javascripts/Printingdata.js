...
function ShowData(number, amount){
    ...
    setTimeout(() => {
        ...
        $.get('http://' + ip + '/api/GetPrintingTableInfo', {'workorderNumber': number}, function (data,status) {
            var rid = data[0].製令單號;      
            ...
            $.get('http://' + ip + '/api/GetPrintingTableList', {'製令單號': rid, '派工單編號': number}, function (data,status) {
        ...
        for(var i=0;i < 15; i++){
            if(i < data.length){
                ...
                var content = 
                    `
                    <td style="width: 5.3333%;">${data[i].總數量}</td>
                    `;
            }else{
                var content = 
                    `
...
async function delete_button(numbers) {
    ...
    var percent = 0
    if (confirm(msg) == true) {
        $.get('http://' + ip + '/api/GetPrintingTableInfo', {'workorderNumber': numbers}, function (data,status) {
            for(var i = 0;i< data.length;i++){
                console.log(data[i]);
                $.get('http://' + ip + '/api/UndoPrintingTable', { '數量': data[i].總數量, '完成數量': 0,'派工時間': null,'派工單編號': null, '製令單號': data[i].製令單號, '派工單編號old': numbers, '尺寸': data[i].尺寸}, function (res) {
                    
                }); // UPDATE printingtable SET 數量 = ? , 完成數量 = ? , 派工時間 = ? , 派工單編號 = ? WHERE 製令單號 = ? AND 派工單編號 = ?
            // 用於回復派工前的數量
            }
            /*
...
// function printmultipleqrcode(queue){
//     console.log(queue)
...
//     document.getElementById("qrcode").setAttribute("id", "done");
// }
