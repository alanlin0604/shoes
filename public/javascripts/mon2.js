    ...
    $.get('http://' + ip + '/api/getDB6', {'a': rid}, function (data,status) {
        for (var i = 0; i < data.length; i++){
            $.get('http://' + ip + '/api/getDB4', {'a': rid,'b': data[i].部位名稱}, function (data,status) {
                var allsum = 0;
                for (var i = 0; i < data.length; i++){
                    allsum += parseInt(data[i].完成數量);
                    console.log(data[i].部位 + ' 完成數量 ' + data[i].完成數量)
            ...
            $.get('http://' + ip + '/api/getDB6total', {'a': rid,'b': data[i].部位名稱}, function (data,status) {
                //console.log('getDB6total')
                for (var i = 0; i < data.length; i++){
                    persent = parseFloat(data[i].回報數量/data[i].總數量);
                    persent = persent.toFixed(0);
                    $.post('http://' + ip + '/api/updateDB6now', {'目前進度': persent,'製令單號': data[i].製令單號,'部位名稱': data[i].部位名稱}, function (res) {
                        console.log('data[i].回報數量/data[i].總數量' + persent);
...
function aa() {
    ...
    function aaaa(data) {
        //console.log(data);
        var d = 'http://' + ip + '/api/getDB6total?a=' + data.製令單號 + '&b=' + data.部位名稱;
        //抓對應總數量
        $.get(d, function (s3, status) {
            for (var i = 0; i < s3.length; i++) {
                ...
                $.post('http://' + ip + '/api/updateDB6now', { '目前進度': p, '製令單號': data.製令單號, '部位名稱': data.部位名稱 }, function (res) {
    ...
    function getList(data) {
        ...
        <td style="width:  25%;;">${data.部位名稱}</td>
