...
function getGroupdispatchList(data) {
    ...
    `;
        ...
        //<td style="width: 10%; text-align: center;">${data.完成狀態}</td>
        var content =
            `
            <tr>
...
async function getbuttonGroupdispatchsize() {
    ...
    //這邊是上面用滑鼠拉的部分(尺寸、數量)
    var s = 'http://' + ip + '/api/selectgetoutsole?s=' + rid + '&s1=' + s1;
...
function getbuttonGroupdispatch() { //顯示內容
    ...
    $.get('http://' + ip + '/api/getoutsolesystem', { 's': rid}, function (data, status) {
    ...
    if (confirm(msg) == true) {
        ...
        for (var i = 0; i < selectedEls.length; i++) {
            ...
            // }
             ...
             //outsole nfc
            $.get('http://' + ip + '/api/updateoutsole', {'派工編號': checkValue,'派工數量': 0 ,'製令單號': rid ,'部位名稱': s1 ,'尺寸': id1 }, function (res) {
            });
