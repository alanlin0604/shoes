...
function newList(data) {
    ...
    if(data.列印狀態 == "列印完成"){
        ...
        <table class="table">
                    ...
                    <a href="針車派工pdf.html?nfcid=${data.派工單編號}" target="_blank" style="text-decoration:none;">
...
function printpdf(){
    var pdfip = 'http://'+ ip +'/針車派工pdf.html?nfcid';
    check = $("input[name='batch_print']:checked")
    for (var i = 0; i < check.length ; i++) {
    console.log(check[i].value)
    pdfip += '=' + check[i].value;
    // urls.push("http://127.0.0.1/針車派工pdf.html");
    }
    if (check.length == 1) pdfip = "http://127.0.0.1/針車派工pdf.html?nfcid="+ check[0].value
    parent.open(pdfip);
